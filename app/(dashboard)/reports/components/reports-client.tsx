...
const tabs = [
  ...
  { id: "transactions", label: "Transactions", icon: Receipt, color: "text-orange-500" },
  { id: "complaints", label: "Complaints", icon: MessageSquare, color: "text-yellow-500" },
  { id: "notifications", label: "Notifications", icon: Bell, color: "text-indigo-500" },
  { id: "settings", label: "System Config", icon: Settings, color: "text-gray-500" },
]
...
export function ReportsClient({
  ...
  const monthlyLoansData = useMemo(() => {
    ...
    loans.forEach((l) => {
      ...
      const key = new Date(l.created_at).toLocaleString("default", { month: "short" })
      if (months[key]) {
  ...
  const monthlySavingsData = useMemo(() => {
    ...
    return Object.entries(months).map(([month, balance]) => ({ month, balance }))
  }, [savings])
  ...
  const memberGrowthData = useMemo(() => {
    ...
    members.forEach((m) => {
      ...
      const key = new Date(m.joined_at).toLocaleString("default", { month: "short" })
  ...
  const topSavers = useMemo(() => {
    return [...savings].sort((a, b) => b.balance - a.balance).slice(0, 10)
  }, [savings])

  const handleExcelExport = (type: string, data: any[], columns: Record<string, string>) => {
    const formatted = data.map((item) => {
      const obj: Record<string, any> = {}
      Object.entries(columns).forEach(([key, label]) => {
        let value = item[key]
        if (key.includes("amount") || key.includes("balance") || key.includes("payment")) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        {[
          { title: "Total Savings", value: formatUGX(stats.totalSavings), description: `${stats.savingsCount} accounts`, icon: PiggyBank, accentColor: "#10b981" },
          { title: "Total Disbursed", value: formatUGX(stats.totalLoansAmount), description: `${stats.totalLoansCount} loans`, icon: Banknote, accentColor: "#3b82f6" },
          { title: "Outstanding", value: formatUGX(stats.activeLoansAmount), description: `${stats.activeLoansCount} active`, icon: TrendingUp, accentColor: "#a855f7" },
          { title: "Expected to Receive", value: formatUGX(stats.totalExpectedAmount), description: "Including interest", icon: ArrowUpRight, accentColor: "#f97316" },
          { title: "Pending Fines", value: formatUGX(stats.pendingFinesAmount), description: `${stats.pendingFinesCount} pending`, icon: AlertCircle, accentColor: "#ef4444" },
          { title: "Open Complaints", value: stats.openComplaints, description: "Awaiting resolution", icon: MessageSquare, accentColor: "#eab308" },
        ].map((card, i) => (
          <div
            key={card.title}
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                {[
                  ...
                  { label: "Active Members", value: stats.activeMembers, color: "text-green-500" },
                  { label: "Total Loans", value: stats.totalLoansCount, color: "text-purple-500" },
                  { label: "Active Loans", value: stats.activeLoansCount, color: "text-orange-500" },
                  { label: "Settled Loans", value: stats.settledLoansCount, color: "text-gray-500" },
                  { label: "Total Fines", value: stats.finesCount, color: "text-red-500" },
                ].map((item) => (
                  <Card key={item.label}>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => handleExcelExport("loans", loans.map(l => ({ loan_ref: l.loan_ref, member_name: l.member_name, amount: l.amount, balance: l.balance, status: l.status, due_date: l.due_date })), { loan_ref: "Loan Ref", member_name: "Member", amount: "Amount", balance: "Balance", status: "Status", due_date: "Due Date" })}>
                    <Download className="h-4 w-4 mr-2" />
                    Excel
                  </Button>
                  <ReportPdfButton type="loans" stats={stats} loans={loans} label="PDF" />
                </div>
              </div>
              <Card>
                        ...
                        {loans
                          ...
                          .map((loan) => (
                            <TableRow key={loan.id} className="hover:bg-muted/30">
                              <TableCell className="font-mono text-sm">{loan.loan_ref}</TableCell>
                              <TableCell>
                                <div>
                                  <p className="font-medium">{loan.member_name}</p>
                                  <p className="text-xs text-muted-foreground">{loan.member_code}</p>
                                </div>
                              </TableCell>
                              <TableCell>{formatUGX(loan.amount)}</TableCell>
                              <TableCell>{formatUGX(loan.balance)}</TableCell>
                              <TableCell>{formatUGX(loan.monthly_payment ?? 0)}</TableCell>
                              <TableCell>
                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                                  style={{ backgroundColor: `${loanStatusColors[loan.status]}20`, color: loanStatusColors[loan.status] }}>
                                  {loan.status}
                                </span>
                              </TableCell>
          ...
          {activeTab === "savings" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => handleExcelExport("savings", savings.map(s => ({ member_name: s.member_name, account_number: s.account_number, balance: s.balance, account_type: s.account_type, is_locked: s.is_locked })), { member_name: "Member", account_number: "Account No", balance: "Balance", account_type: "Type", is_locked: "Locked" })}>
                    <Download className="h-4 w-4 mr-2" />
                    Excel
                  </Button>
                  <ReportPdfButton type="savings" stats={stats} savings={savings} label="PDF" />
                </div>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                <Card>
                  <CardContent className="pt-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-green-500">{formatUGX(stats.totalSavings)}</p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-4 text-center">
                ...
                <Card>
                  <CardContent className="pt-4 text-center">
                    ...
                    <p className="text-2xl font-bold">{formatUGX(stats.savingsCount > 0 ? Math.floor(stats.totalSavings / stats.savingsCount) : 0)}</p>
                  </CardContent>
                </Card>
              </div>
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base">Top Savers Leaderboard</CardTitle>
                </CardHeader>
                <CardContent>
                        ...
                        {topSavers
                          ...
                          .map((s, i) => (
                            <TableRow key={s.id} className="hover:bg-muted/30">
                              <TableCell>
                                <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${i === 0 ? "bg-yellow-100 text-yellow-700" : i === 1 ? "bg-gray-100 text-gray-600" : i === 2 ? "bg-orange-100 text-orange-700" : "bg-muted text-muted-foreground"}`}>
                                  {i + 1}
                                </span>
                              </TableCell>
                              <TableCell>
                                <div>
                                  ...
                                  <p className="text-xs text-muted-foreground">{s.member_code}</p>
                                </div>
                              </TableCell>
                              <TableCell className="font-mono text-sm">{s.account_number}</TableCell>
                              <TableCell className="font-semibold text-green-600">{formatUGX(s.balance)}</TableCell>
                              <TableCell>
                                <Badge variant={s.is_locked ? "destructive" : "default"}>
          ...
          {activeTab !== "overview" && activeTab !== "loans" && activeTab !== "savings" && (
            <Card>
              <CardContent className="pt-12 pb-12 text-center">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm text-muted-foreground">
                    The {tabs.find(t => t.id === activeTab)?.label} report is currently under development.
                  </p>
                </div>
              </CardContent>
            </Card>
