...
export async function POST(req: Request) {
  try {
    ...
    const { account_id, amount, tx_ref } = await req.json()
    if (!account_id || !amount || amount < 1000) {
      return NextResponse.json(
        { error: "Minimum deposit is UGX 1,000" },
        { status: 400 }
      )
    }
    ...
    await db.insert(notifications).values({
      ...
      body: `${member?.full_name} (${member?.member_code}) deposited UGX ${(amount / 100).toLocaleString()} via mobile money. New balance: UGX ${(newBalance / 100).toLocaleString()}.`,
      type: "in_app",
      status: "sent",
      priority: "normal",
      channel: "in_app",
